...
class DerivationService
{
    public function derive(CanonicalRelMonDto $relmon, MonetaryBasisInterface $basis): DerivedResult
    {
        return match ($relmon->getProtocolIdentifier()->getDeterminismLevel()) {
            DeterminismLevel::DL1 => $this->deriveDeterminismLevelOne($relmon, $basis),
            DeterminismLevel::DL2 => $this->deriveDeterminismLevelTwo($relmon, $basis),
            DeterminismLevel::DL3 => $this->deriveDeterminismLevelThree($relmon, $basis),
            default => throw new DerivationException('Unsupported determinism level.'),
        };
    }
    ...
    ): DerivedResult
    {
        ...
        } elseif (!is_null($net) && !is_null($gross) && $gross < $net && $net >= 0) {
            throw new DerivationException('Gross must be greater than or equal to net.');
        }

        $taxRateDivisor = 100 * (10 ** $basis->getTaxRatePrecision());

        if ($relmon->getRoundingApplication() === RoundingApplication::TAX) {
            if (!is_null($net)) {
    ...
    ): DerivedResult
    {
        ...
        if (
            ...
            throw new DerivationException('Gross must be greater then or equal to net (arithmetic check).');
        }
